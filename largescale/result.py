...
def custom_format2(y, pos):
    return f'{y/100:.1f}'

def custom_format3(y, pos):
    if y == 0:
        return '0'
    return f'{y/10000000:.0f}e7'

def custom_format4(y, pos):
    if y == 0:
        return '0'
    return f'{y/100000:.0f}e5'
...
def draw_prepared_result(results):
    ...
    plt.title('(B) Large-scale', fontweight='bold')
    ...
    axes[0].set_title('(A) Total handover count', fontweight='bold')
    axes[0].set_ylabel('Handover count', fontweight='bold')
    axes[0].yaxis.set_major_formatter(plt.FuncFormatter(custom_format4))
    axes[0].grid(True)
    ...
    axes[1].set_title('(B) Average serving time', fontweight='bold')
    axes[1].set_ylabel('Time (s)', fontweight='bold')
    axes[1].yaxis.set_major_formatter(plt.FuncFormatter(custom_format))
    axes[1].grid(True)
    ...
    # 绘制子图
    fig, axes = plt.subplots(1, 2, figsize=(8, 3.25))  # 1行2列的子图，设置总大小为6x3

    axes[0].bar(sorted_labels[:len(sorted_mean)], sorted_mean, color=colors, edgecolor='black')
    The_min = sorted_mean[0]
    axes[0].axhline(y=The_min, color='yellow', linestyle='dotted', linewidth=2)
    axes[0].set_title('(A) Busy time slot average', fontweight='bold')
    axes[0].set_ylabel('Signalling count', fontweight='bold')
    axes[0].grid(True)
    for tick in axes[0].xaxis.get_major_ticks():
        tick.label.set_fontweight('bold')
    for tick in axes[0].yaxis.get_major_ticks():
    ...
    The_min = sorted_data[0]
    axes[1].axhline(y=The_min, color='yellow', linestyle='dotted', linewidth=2)
    axes[1].set_title('(B) Busy time slot share balance', fontweight='bold')
    axes[1].set_ylabel('Coefficient of variation', fontweight='bold')
    axes[1].grid(True)
    for tick in axes[1].xaxis.get_major_ticks():
        tick.label.set_fontweight('bold')
    ...
    The_min = sorted_data[0]
    axes[1].axhline(y=The_min, color='yellow', linestyle='dotted', linewidth=2)
    axes[1].set_title('(B) Total reservation time', fontweight='bold')
    axes[1].set_ylabel('Time (s)', fontweight='bold')
    #axes[1].set_yscale('log')
    axes[1].yaxis.set_major_formatter(plt.FuncFormatter(custom_format3))
    axes[1].grid(True)
    for tick in axes[1].xaxis.get_major_ticks():
        tick.label.set_fontweight('bold')
    for tick in axes[1].yaxis.get_major_ticks():
    ...
    The_min = sorted_data[0]
    axes[0].axhline(y=The_min, color='yellow', linestyle='dotted', linewidth=2)
    axes[0].set_title('(A) Average decouple time', fontweight='bold')
    axes[0].set_ylabel('Time (s)', fontweight='bold')
    axes[0].yaxis.set_major_formatter(plt.FuncFormatter(custom_format2))
    axes[0].grid(True)
    for tick in axes[0].xaxis.get_major_ticks():
        tick.label.set_fontweight('bold')
    for tick in axes[0].yaxis.get_major_ticks():

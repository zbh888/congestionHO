...
def draw_prepared_result(results):
    ...
    colors = ['red', 'yellow'] + ['skyblue'] * (len(sorted_data) - 2)
    plt.figure(figsize=(4, 3.25))
    plt.axhline(y=10, color='red', linestyle='dotted', linewidth=2)
    plt.axhline(y=sorted_data[1], color='yellow', linestyle='dotted', linewidth=2)
    plt.bar(sorted_labels[:len(sorted_data)], sorted_data, color=colors, edgecolor='black')
    plt.xticks(fontweight='bold')
    plt.yticks(fontweight='bold')
    plt.title('(A) Small-scale', fontweight='bold')

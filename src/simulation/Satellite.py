...
class Satellite(Base):
    ...
    def prepare_my_load_prediction(self):
        ...
        return (self.within_one_slot_load_priority,
                self.predicted_my_load[now:now + LOAD_WINDOW_SIZE + 1],
                self.predicted_my_load_potential[now:now + LOAD_WINDOW_SIZE + 1])
    ...
    def prepare_other_load_prediction(self, satid):
        ...
        if satid not in self.load_aware:
            ...
            candidate_load = self.load_aware[satid][1][1][now - candidate_time:]
            candidate_load_potential = self.load_aware[satid][1][2][now - candidate_time:]
    ...
    def increment_other_cur_load(self, amount):
        now = round(self.env.now)
        # print(f"{self.identity},{self.env.now} [{time}, + {amount}]: real load")
        self.predicted_my_load[now] += amount
    ...
    def update_other_priority_load(self, satid, priority_load):
        now = round(self.env.now)
        if satid not in self.load_aware:
            self.load_aware[satid] = (now, priority_load)
    ...
    def handle_messages(self):
        while True:
            ...
            if task not in [MEASUREMENT_REPORT, RANDOM_ACCESS, RRC_RECONFIGURATION_COMPLETE, PATH_SWITCH_REQUEST_ACK]:
                self.load_aware[data['from']] = (now, data['priority_load'])
            ...
            # ================================================ Target + Candidate
                ...
                self.increment_my_load_potential(now + condition.access_delay,
                ...
                self.increment_my_load_potential(now + condition.ue_utility,
            ...
            # ================================================ Source
                ...
                if len(self.condition_record[ueid]) == len(self.candidates_record[ueid]):
                    ...
                    self.increment_my_load(now + delay, TARGET_HANDOVER_SUCCESS_SIGNALLING_COUNT_ON_SOURCE)
            ...
            # ================================================ Target
                ...
                self.increment_my_load(now,
                                                SOURCE_HANDOVER_REQUEST_SIGNALLING_COUNT_ON_CANDIDATE)
                self.decrease_my_load_potential(now,
                                                 SOURCE_HANDOVER_REQUEST_SIGNALLING_COUNT_ON_CANDIDATE)
                assert (self.current_assigned_slot.include(ueid))
                assert (self.current_assigned_slot.time == now)
    ...
    def prepare_condition(self, ueid, sourceid, candidates, utilities):
        ...
        if now + delay < self.DURATION:
            assert (self.coverage_info[ueid, self.identity, now + delay] == 1)
        expected_leaving_time = ue_utility + now
    ...
    def decide_delay(self, ueid, sourceid, candidates, utilities):
        now = round(self.env.now)
        assert (self.access_Q.counter - self.access_Q.max_access_slots == now)
